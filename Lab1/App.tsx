...
import {ShapeType, Dimensions, CalculationHistory, QuadraticRoots} from "./types";
...
export default function App() {
    ...
    const [roots, setRoots] = useState<QuadraticRoots | null>(null);
        ...
        try {
            switch (shape) {
                ...
                case 'circle':
                    ...
                    calculation = `π × ${dimensions.radius}²`;
                    break;
                case 'quadratic':
                    if (!dimensions.a || !dimensions.b || !dimensions.c) {
                        throw new Error("Все коэффициенты должны быть заполнены")
                    }
                    ...
                    if (discriminant < 0) {
                        throw new Error("Дискриминант отрицательный. Действительных корней нет.")
                    }
                    ...
                    calculation = `D = b² - 4ac = ${discriminant.toFixed(2)}`;
                    calculatedArea = discriminant; // Для совместимости с историей
                    break;
                default:
                    ...
                    <View style={styles.buttonGroup}>
                        <TouchableOpacity style={styles.calculateButton}
                            ...
                            <Text style={styles.calculateButtonText}>
                                {shape === 'quadratic' ? 'Найти корни' : 'Рассчитать'}
                            </Text>
                        </TouchableOpacity>
                    ...
                    {shape === 'quadratic' && roots && (
                        <View style={styles.resultSection}>
                            ...
                            <Text style={styles.discriminantText}>
                                Дискриминант: D = {roots.discriminant}
                            </Text>
                            <Text style={styles.rootsText}>x₁ = {roots.x1}</Text>
                            <Text style={styles.rootsText}>x₂ = {roots.x2}</Text>
                            <Text style={styles.shapeName}>{currentConfig.name}</Text>
                        </View>
                    )}

                    {shape !== 'quadratic' && area !== null && (
                        <View style={styles.resultSection}>
                            <Text style={styles.resultTitle}>Результат:</Text>
                            <Text style={styles.areaText}>Площадь: {area}</Text>
                            <Text style={styles.shapeName}>{currentConfig.name}</Text>
                        </View>
                    )}
